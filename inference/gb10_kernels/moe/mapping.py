"""get_fused_mapping: build the (token,topk) -> expanded-position scatter map.

Adapted from deepseek-ai/TileKernels tile_kernels/moe/get_fused_mapping_kernel.py
(MIT, Copyright (c) 2026 DeepSeek). Imports rewritten to be relative.
"""
import os
...
from ..utils import align, get_num_sms
...
def get_fused_mapping(
    ...
    num_experts: int,
    num_expanded_tokens: int = 0,
    alignment: int = 32,
    force_no_sync: bool = False,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor,
           torch.Tensor, torch.Tensor, torch.Tensor, list[int]]:
    """Build fused MoE routing mappings.

    Args:
      topk_idx: int64 [num_tokens, num_topk] expert indices (-1 = no-route).
      num_experts: total experts in this layer (e.g. 256 or 128).
      num_expanded_tokens: size of the expanded buffer; 0 = estimate then trim.
      alignment: per-expert segment alignment (must be >= grouped_fp4_gemm block_M=32).
      force_no_sync: skip the host sync that trims padding.

    Returns:
      (pos_to_expert, pos_to_token, pos_to_token_topk, token_topk_to_pos,
       expert_start, expert_end, num_tokens_per_expert, num_tokens_per_expert_list).
    """
    ...
    kernel = _get_kernel(num_experts, num_topk, alignment, num_sms)
    kernel(
        topk_idx, pos_to_expert, pos_to_token, pos_to_token_topk,
        token_topk_to_pos, expert_start, expert_end,
        num_tokens_per_expert, num_experts_per_sm,
    )
